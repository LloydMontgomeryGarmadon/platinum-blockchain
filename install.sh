...
if [ "$(uname -m)" = "armv7l" ]; then
  echo ""
  echo "WARNING:"
  echo "The  Platinum Blockchain requires a 64 bit OS and this is 32 bit armv7l"
  exit 1
fi
...
install_python3_and_sqlite3_from_source_with_yum() {
  ...
  cd "$TMP_PATH"
  # Install sqlite>=3.37
  # yum install sqlite-devel brings sqlite3.7 which is not compatible with platinum
  ...
  # yum install python3 brings Python3.6 which is not supported by platinum
  cd ..
...
if [ "$PYTHON_MAJOR_VER" -ne "3" ] || [ "$PYTHON_MINOR_VER" -lt "7" ] || [ "$PYTHON_MINOR_VER" -ge "12" ]; then
  echo "Platinum requires Python version >= 3.7 and  < 3.12.0" >&2
...
echo ""
echo "Platinum blockchain install.sh complete."
echo ""
echo "To install the GUI run '. ./activate' then 'sh install-gui.sh'."
echo ""
echo "Type '. ./activate' and then 'platinum init' to begin."